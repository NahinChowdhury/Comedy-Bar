...
function getChildrenComments(postId: string, parentCommentId: string, commentsFound: CommentInterface[]): any{
    ...
    const matchingComments = commentsFound.filter(
        (comment: CommentInterface) => comment.PARENT_COMMENT_ID === parentCommentId
    );
