...
export const Chat:FunctionComponent = () => {
	...
	const [otherMember, setOtherMember] = useState<{sender: string, senderFullName: string} | undefined>(undefined);
	...
	const requestData = async () => {
		...
		console.log(chatId)
		await axios.get(`/api/user/chat/${chatId}/getOtherMember`)
			.then(res => {
				const {otherMember} = res.data;
			...
			await axios.post(`/api/user/chat/${chatId}/createMessage`, {details: messageToSend})
				.then(res => {
					...
					socket.emit('send_message', { 
						...
						senderFullName: messageCreated.senderFullName,
			...
			await axios.put(`/api/user/chat/${chatId}/editMessage`, {messageId: messageToBeEditedID, messageDetails: messageToBeEditedDetails})
				.then(res => {
					...
					socket.emit('edit_message', { 
						...
						senderFullName: messageEdited.senderFullName,
	...
	const renderPlainMessage = (message: ChatMessageInterface, color: string) => {
		return(
			<>
				<div>
				<span style={{ color: color }}> <strong>{`${message.senderFullName}: `}</strong> </span> 
	...
	return (
		...
		<div style={{ height: '80vh', display: 'flex', flexDirection: 'column' }}>
			...
			{otherMember !== undefined  && <h1>Chat with {otherMember.senderFullName || otherMember.sender}</h1>}
